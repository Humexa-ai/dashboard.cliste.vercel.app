...
export default function LoginCardSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    setLoading(true);
    const { error } = await supabase.auth.signInWithPassword({ identifier: email, email, password } as any);
    if (error) {
      setLoading(false);
      setError(error.message || "Sign in failed. Please try again.");
      return;
    }
    // Wait for session to be available before navigation to avoid race with redirects
    for (let i = 0; i < 10; i++) {
